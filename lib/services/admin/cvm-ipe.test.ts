...
describe("cvm-ipe service", () => {
  ...
  it("forwards validation_status to the disclosures list", async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({
    ...
    await listIPEDisclosures({ page: 1, page_size: 20, validation_status: "valid" });

    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:8001/api/v1/admin/cvm/ipe/disclosures?page=1&page_size=20&validation_status=valid",
