...
export default function ITRDFPCompanyExplorerPage() {
  ...
  const filingsQuery = useQuery({
    queryKey: ["cvm", "itr-dfp", "company-filings", cdCvm],
    queryFn: () =>
      listITRDFPFilings({
        cd_cvm: Number(cdCvm),
        limit: 200,
  ...
  const reconciliationQuery = useQuery({
    ...
    queryFn: () =>
      getITRDFPReconciliation(cdCvm, selectedFiling?.reference_date ?? "", {
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <CardTitle>Explorer ITR/DFP · empresa {cdCvm}</CardTitle>
              <CardDescription>
