...
const columns: DataTableColumn<FREFilingSummary>[] = [
  {
    key: "ref",
    header: "Referencia",
    render: (row) => (
  ...
  },
  {
    ...
    render: (row) => (
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">cd_cvm {row.cd_cvm ?? "—"} · CNPJ {row.cnpj_companhia}</p>
      </div>
    ),
  },
  {
  ...
  },
  {
    ...
    render: (row) => (
      <Link
        ...
        href={`/cvm/fre/filings/${encodeURIComponent(row.id_documento)}`}
      >
        Abrir
      </Link>
    ),
...
export default function FREPage() {
  ...
  const syncStatusQuery = useQuery({
    queryKey: ["cvm", "fre", "sync-status"],
    queryFn: getFRESyncStatus,
  });
  ...
  const triggerSyncMutation = useMutation({
    mutationFn: () =>
      triggerFRESync({ year: Number(syncYear), force: forceSync }),
    onSuccess: (data) => {
      toast.success(`Sync FRE agendado (task_id ${data.task_id}).`);
      queryClient.invalidateQueries({ queryKey: ["cvm", "fre"] });
    },
    ...
    <div className="flex flex-col gap-4">
        ...
        <CardHeader>
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>Disparar sync de FRE?</AlertDialogTitle>
                  <AlertDialogDescription>
                    Envia <span className="font-mono">POST /admin/cvm/fre/sync</span> para o ano informado.
                  </AlertDialogDescription>
                </AlertDialogHeader>
        ...
        <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <Card className="metric-tile">
            <CardHeader>
              ...
              <CardTitle>{status?.total_filings ?? "—"}</CardTitle>
            </CardHeader>
          </Card>
          <Card className="metric-tile">
            <CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Numero de linhas em cada uma das 21 secoes do FRE — ajuda a sentir saude do dataset.
          </CardDescription>
        </CardHeader>
        <CardContent>
